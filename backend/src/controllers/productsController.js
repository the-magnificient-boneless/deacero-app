...
exports.getAllProductsPagination = async (req, res) => {
  try {
    ...
    if (filters.category) {
      filter.category = filters.category; // Exact match for category
    }

    // Filtrar por rango de precio
    if (filters.minPrice || filters.maxPrice) {
      filter.price = {};
      if (filters.minPrice) filter.price.$gte = filters.minPrice; // Greater than or equal to minPrice
      if (filters.maxPrice) filter.price.$lte = filters.maxPrice; // Less than or equal to maxPrice
    }

    // Filtrar por rango de stock
    if (filters.minStock || filters.maxStock) {
      filter.stock = {};
      if (filters.minStock) filter.stock.$gte = filters.minStock; // Greater than or equal to minStock
      if (filters.maxStock) filter.stock.$lte = filters.maxStock; // Less than or equal to maxStock
    }
